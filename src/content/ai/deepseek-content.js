...
class DeepSeekChatAssistant {
  ...
  log(...args) {
    try {
      if (this.debugPanel) {
        this.debugPanel.log('[DeepSeek]', ...args);
      }
      console.log('[DeepSeek]', ...args);
    } catch (error) {
      // 防止日志系统本身出错
      console.error('[DeepSeek日志错误]', error);
    }
  }
  ...
  error(...args) {
    try {
      if (this.debugPanel) {
        this.debugPanel.log('❌ [DeepSeek错误]', ...args);
      }
      console.error('[DeepSeek错误]', ...args);
    } catch (error) {
      console.error('[DeepSeek日志错误]', error);
    }
  }
  ...
  init() {
    try {
      ...
      window.postMessage({ type: 'AI_ASSISTANT_READY', aiName: 'DeepSeek' }, '*');
...
window.addEventListener('error', (event) => {
  console.error('[DeepSeek全局错误]', event.error);
});

// 页面加载完成后初始化助手
if (document.readyState === 'complete' || document.readyState === 'interactive') {
  setTimeout(() => {
    window.deepseekChatAssistant = new DeepSeekChatAssistant();
  }, 1000);
} else {
